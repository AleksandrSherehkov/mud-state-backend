...
function expectPublicUserShape(
  ...
  expect((v.email as string).length).toBeGreaterThan(3);
  expect(Number.isNaN(Date.parse(v.createdAt as string))).toBe(false);

  expect('password' in v).toBe(false);
}

describe('Users E2E — GET /users/id/:id', () => {
  ...
  it('404: ADMIN token -> Not Found when user does not exist', async () => {
    ...
    const res = await request(app.getHttpServer())
      .get(`${basePath}/users/id/cl_nonexistent_user_id_123`)
  ...
  it('200: ADMIN token -> returns PublicUserDto by id', async () => {
    ...
    const userEmail = `target_${Date.now()}@e2e.local`;
    ...
    const res = await request(app.getHttpServer())
      .get(`${basePath}/users/id/${target.id}`)
    ...
    expect(body.email).toBe(userEmail.toLowerCase());
  });
});
